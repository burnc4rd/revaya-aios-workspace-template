...
informed content decisions. Pulls from:
  - Recent published content (from published_content table)
  - Recent meetings (from data/intel.db IntelOS database if available)
...
Usage:
    python scripts/content/context_aggregator.py              # Print context summary
    python scripts/content/context_aggregator.py --full       # Print full formatted context
...
INTEL_DB_PATH = WORKSPACE_ROOT / "data" / "intel.db"


def _get_intel_connection() -> sqlite3.Connection | None:
    """Try to connect to the IntelOS database for richer context (meetings, Slack).
    Returns None if IntelOS is not installed."""
    if not INTEL_DB_PATH.exists():
        return None
    conn = sqlite3.connect(str(INTEL_DB_PATH))
    ...
    if "meetings" in tables or "slack_messages" in tables:
        return conn
    conn.close()
    return None
...
def build_context_window(days: int = 7) -> dict:
    ...
    Returns:
        {
            "recent_content": [{"title", "platform", "published_date", "url", "description", "metrics"}],
    ...
    try:
        rows = conn.execute(
            """SELECT platform, title, published_date, url, description, metrics_json
        ...
        for r in rows:
            ...
            if r["metrics_json"]:
                try:
                    import json
                    entry["metrics"] = json.loads(r["metrics_json"])
                except Exception:
                    pass
    ...
    # --- Recent meetings (from IntelOS intel.db if available) ---
    recent_meetings = []
    intel = _get_intel_connection()
    if intel:
        try:
            tables = [
                r[0]
                for r in intel.execute(
                    "SELECT name FROM sqlite_master WHERE type='table'"
                ).fetchall()
            ]
            if "meetings" in tables:
                meetings = intel.execute(
...
def format_context_for_prompt(context: dict) -> str:
    ...
    if content:
        for c in content:
            ...
            if metrics.get("impressions"):
                metrics_str += f" — {metrics['impressions']:,} impressions"
            if metrics.get("reactions"):
                metrics_str += f", {metrics['reactions']} reactions"
            sections.append(
                f"\n**{c['title']}** [{platform}] ({c.get('published_date', '?')}){metrics_str}"
            )
    ...
    if meetings:
        ...
        sections.append(
            "No meetings in this window. (IntelOS collects meetings daily — check data/intel.db.)"
        )
...
if __name__ == "__main__":
    if not DB_PATH.exists():
        ...
        print("Run 'python scripts/content/db.py' first to initialize.")
        sys.exit(1)
