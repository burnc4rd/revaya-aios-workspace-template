...
Usage:
    from scripts.content.writer import write_content_idea, write_developed_idea, update_status
"""
...
def update_status(conn: sqlite3.Connection, idea_id: int, status: str, **kwargs):
    ...
    assigned_thumbnail, airtable_record_id, published_url.
    """
...
def log_published_content(conn: sqlite3.Connection, entry: dict) -> int:
    ...
    (e.g., LinkedIn posts, YouTube videos). Also called by sync_airtable.py
    to seed historical posts.
    """
    cursor = conn.execute(
